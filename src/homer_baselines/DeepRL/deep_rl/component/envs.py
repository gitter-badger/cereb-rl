...
import baselines
import baselines.homer



def build_env_homer(horizon=10, seed=1, log_dir = './log/'):
    import homer_envs
    ...
    if os.getenv('PT_OUTPUT_DIR') is not None:
        config['trace_folder'] = os.getenv('PT_OUTPUT_DIR') + '/traces/'
...
def make_env(env_id, seed, rank, episode_life=True, horizon=4, dimension=30):
    def _thunk():
        random_seed(seed)
