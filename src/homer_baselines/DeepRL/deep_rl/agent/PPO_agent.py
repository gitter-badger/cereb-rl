...
class PPOAgent(BaseAgent):
    def __init__(self, config):
        ...
        elif config.bonus_type == 'oracle-cnts':
            self.counts = dict()
    ...
    def step(self):
        ...
        for _ in range(config.rollout_length):
            ...
            if config.bonus_type == 'rnd':
                ...
                rewards += config.reward_bonus_normalizer(config.bonus_coeff*rnd_loss.detach().cpu().numpy())
            elif config.bonus_type == 'oracle-cnts':
                reward_bonus = []
                latent_states = [i['state'] for i in info]
                for state in latent_states:
                    if state not in self.counts.keys():
                        self.counts[state] = 0
                    else:
                        self.counts[state] += 1
                for state in latent_states:
                    reward_bonus.append(1.0/math.sqrt(1+self.counts[state]))
                rewards += config.reward_bonus_normalizer(config.bonus_coeff*np.array(reward_bonus))
