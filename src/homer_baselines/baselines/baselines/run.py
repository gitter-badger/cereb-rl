...
def train(args, extra_args):
    ...
    if 'diabcombolock' in args.env:
        total_timesteps = int(args.num_timesteps) * args.horizon
    else:
...
def build_env_homer(horizon=10, seed=1):
    import environments
    config = environments.default_config.default_config()
    config['horizon'] = horizon
    config['noise'] = 'hadamhardg'
    config['seed'] = seed
    environments.environment_wrapper.GenerateEnvironmentWrapper.adapt_config_to_domain('diabcombolock', config)
    env = environments.environment_wrapper.GenerateEnvironmentWrapper('diabcombolock', config)
    return env


'''
def build_env_homer(horizon=10):
    import baselines.homer.environment_wrapper
    ...
    with open("baselines/homer/data/diabcombolock/config.json") as f:
        ...
        config["horizon"] = horizon
        config["noise"] = 'hadamhardg'
        baselines.homer.environment_wrapper.GenerateEnvironmentWrapper.adapt_config_to_domain('diabcombolock', config)
    print(json.dumps(config, indent=2))        
    baselines.homer.environment_wrapper.GenerateEnvironmentWrapper.adapt_config_to_domain('diabcombolock', config)
    env = baselines.homer.environment_wrapper.GenerateEnvironmentWrapper('diabcombolock', config)
...
def get_env_type(args):
    if 'Lock-v0' in args.env:
        return 'combolock', args.env
    elif 'maze' in args.env:
        return 'maze', args.env
    elif 'diabcombolock' in args.env:
        return 'diabcombolock', args.env
    env_id = args.env
...
def main(args):
    ...
    if args.save_path is not None and rank == 0:
        ...
        '''
        if 'Lock-v0' in args.env:
            env.save_environment(save_path + '/env', 0)
            pickle.dump(env.trajectories, open( save_path + "/traj.pkl", "wb" ) )
        '''
