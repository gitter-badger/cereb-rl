        ...
        for it in range(10):
            ...
            for state in states_visited:
                ...
                for action in self.actions:
                    ...
                    # Q(s, a) = R(s, a) + \sum_{s'} T(s'|s,a) max_{a'} Q(s', a')        , Inductive case
                    # Q(s, a) = R(s, a)                                                 , Base Case
    ...
    def refine_model(self, model, episode):
        ...
        for (state, action, new_states) in transitions:
            ...
            if (state, action) not in model:
                model[(state, action)] = CountConditionalProbability()

            model[(state, action)].add(new_states)
    ...
    def train(self, experiment, env, env_name, experiment_name, logger, use_pushover,
        ...
        for it in range(1, self.max_iter + 1):
            ...
            if it % 100 == 0:
                ...
                logger.log("Episodes %d, Number of states visited %d, Max return %f, Mean return %f, Time taken %f sec"
                           % (it, len(states_visited), max_return, mean_return, round(time.time() - time_start, 1)))
                print("Episodes %d, Number of states visited %d, Max return %f, Mean return %f, Time taken %f sec" %
                      (it, len(states_visited), max_return, mean_return, round(time.time() - time_start, 1)))
