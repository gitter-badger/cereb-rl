...
from model.stationary_constant_policy import StationaryConstantPolicy
...
class PathPolicySearch(AbstractPolicySearch):
    ...
    def do_train(config, constants, homing_policy_dataset, env_info, policy_folder_name, actions, horizon,
        ...
        learned_policy, mean_reward, info =  \
            pps.train(homing_policy_dataset, env, actions, horizon, reward_func, homing_policies, logger,
                      tensorboard, debug, use_pushover, reward_id)
    ...
    def train(self, homing_policy_dataset, env, actions, horizon, reward_func, homing_policies, logger, tensorboard,
                  debug=False, use_pushover=False, reward_id=-1):
        ...
        dataset = self._generate_pps_dataset(homing_policy_dataset, reward_func, horizon, reward_id)
