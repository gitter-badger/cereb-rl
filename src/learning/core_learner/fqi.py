...
from learning.core_learner.abstract_policy_search import AbstractPolicySearch
from learning.learning_utils.contextual_bandit_oracle import ContextualBanditOracle
from model.stationary_deterministic_policy import StationaryDeterministicPolicy
...
class FQI(AbstractPolicySearch):
    ...
    def do_train(config, constants, homing_policy_dataset, env_info, policy_folder_name, actions, horizon,
                 encoder_reward_args, homing_policies, logger, debug, use_pushover):
        ...
        learned_policy, mean_reward, _ = fqi.train(homing_policy_dataset, env, actions, horizon, reward_func,
                                                   homing_policies, logger, None, debug, use_pushover)
    ...
    def train(self, replay_memory, env, actions, horizon, reward_func, homing_policies, logger, tensorboard,
                  debug=False, use_pushover=False):
        ...
        for step in range(horizon, 0, -1):
            ...
            contextual_bandit_dataset = self._gather_fqi_samples(
            ...
            if debug:
                logger.log("Step %d CB: Best Test Loss %f, Best Train Loss %f, and Stopping epoch %d" %
